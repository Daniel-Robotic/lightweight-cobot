...
Один рабочий поток владеет `ClientApplication::step()`. После каждого шага он
ожидает ровно один цикл `read → JTC update → write`; `read()` ожидает новый
снимок, `write()` разрешает следующий шаг SDK. Ожидания ограничены 100 мс.
Команда этого расчёта отправляется в следующем шаге SDK: задержка — один цикл,
без EMA и очереди пропущенных точек. Сетевой ввод-вывод остаётся в рабочем потоке.
Launch включает `hardware_synchronization.expect_blocking_read_write` и требует
Controller Manager **4.48.0 или новее** в Jazzy. Это режим
[синхронизации цикла с оборудованием](https://github.com/ros-controls/ros2_control/blob/jazzy/controller_manager/src/ros2_control_node.cpp),
без второго независимого таймера. Период из `robot.fri_cycle_ms` проверяется
по телеметрии Sunrise. До первого UDP-пакета допускается ожидание Sunrise
...
При отсутствии пакетов/команд 100 мс, пропуске timestamp в COMMANDING_ACTIVE,
несовпадении периода, нерастущем timestamp, нечисловой
...
Результаты исходной ревизии: сборка пяти затронутых ROS-пакетов прошла; 14 C++-сценариев
...
Исправление 16.09.2026: воспроизведено удвоение шага 0,001 → 0,002 рад при
перезаписи команды независимыми циклами. Добавлены тесты синхронизации,
запрета перезаписи, остановки/таймаута, периода и пропуска пакетов. Синхронизация
устраняет потерю точек между потоками, но сама по себе не гарантирует отсутствие
задержек планировщика ОС или сети. Исчезновение стука на физическом роботе пока
не подтверждено; исходные журналы испытания не сохранились.
